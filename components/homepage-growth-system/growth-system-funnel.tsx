...
function FunnelDetailCard({ step, priorityImage }: { step: FunnelStep; priorityImage?: boolean }) {
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-border bg-white shadow-sm">
      {step.imageUrl ? (
        <div className="overflow-hidden border-b border-border bg-[#F9F7F4]">
          <Image
            src={step.imageUrl}
  ...
  return (
    <span
      className={cn(
        ...
        isActive ? "bg-primary text-white" : "bg-[#F3F4F6] text-[#6B7280]",
      )}
    >
      {index + 1}
    </span>
        ...
        >
          {content.funnelSteps.map((step, index) => (
            <label
              ...
              className="growth-funnel-mobile-tab flex min-w-0 cursor-pointer touch-manipulation items-center gap-2 rounded-xl border border-border bg-white px-3 py-3 text-left text-[#374151]"
            >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "border-primary bg-[#EFF6FF] text-heading"
                    : "border-border bg-white text-[#374151] hover:border-[#B8BBC2]",
                )}
              >
